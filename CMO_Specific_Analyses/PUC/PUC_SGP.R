###################################################################
####
####	Code for running TCRP (PUC) 2012 Baseline SGP Analyses
####
###################################################################
...
load("Data/PUC_SGP-LONG_ONLY.Rdata")
...
############################################################
	...
	###  Add in the TCRP Coefficient Matrices for 2010 - 2012 to the PUC SGP object.  Need these with sgp.use.my.coefficient.matrices=TRUE
	PUC_SGP@SGP$Coefficient_Matrices <- TCRP_Coefficient_Matrices
	
	PUC_SGP <- analyzeSGP(PUC_SGP, state="CA",
...
###
	...
	#  No GENERAL_MATHEMATICS data for PUC in 2012.
...
names(PUC_SGP@SGP$SGPercentiles)
dim(PUC_SGP@SGP$SGPercentiles$ALGEBRA_I.2012)

PUC_SGP@SGP[["SGPercentiles"]][["ALGEBRA_I.2010"]][['PRIOR_PREF']] <- 1
PUC_SGP@SGP[["SGPercentiles"]][["GENERAL_MATHEMATICS.2010"]][['PRIOR_PREF']] <- 1
PUC_SGP@SGP[["SGPercentiles"]][["SCIENCE.2010"]][['PRIOR_PREF']] <- 1

PUC_SGP@SGP[["SGPercentiles"]][["ALGEBRA_I.2011"]][['PRIOR_PREF']] <- 1
PUC_SGP@SGP[["SGPercentiles"]][["GENERAL_MATHEMATICS.2011"]][['PRIOR_PREF']] <- 1
PUC_SGP@SGP[["SGPercentiles"]][["SCIENCE.2011"]][['PRIOR_PREF']] <- 1

PUC_SGP@SGP[["SGPercentiles"]][["ALGEBRA_I.2012"]][['PRIOR_PREF']] <- 1
# PUC_SGP@SGP[["SGPercentiles"]][["GENERAL_MATHEMATICS.2012"]][['PRIOR_PREF']] <- 1
PUC_SGP@SGP[["SGPercentiles"]][["SCIENCE.2012"]][['PRIOR_PREF']] <- 1
...
###
	...
	Elem.MS.results <- list(
		SGPercentiles = PUC_SGP@SGP[["SGPercentiles"]],
		Goodness_of_Fit = PUC_SGP@SGP[["Goodness_of_Fit"]],
		Knots_Boundaries = PUC_SGP@SGP[["Knots_Boundaries"]]
	)

	##  NULL the parts of the @SGP slot that will be continually replaced by the repeated call to analyzeSGP:
	PUC_SGP@SGP[["SGPercentiles"]] <- NULL
	PUC_SGP@SGP[["Goodness_of_Fit"]] <- NULL
	PUC_SGP@SGP[["Knots_Boundaries"]] <- NULL
...
###  Next year we should use specific grades (7th and 8th) for grade level content area priors (MATH, ELA)
	...
	PUC_SGP@Data[['TMP_GRADE']] <- PUC_SGP@Data[['GRADE']] 
	PUC_SGP@Data[['GRADE']] <- 'EOCT'  
...
##  See the file PUC_Course_Progressions_Math+ELA_*.R for details.
...
#  First set up a group of empty lists to collect the new results.
	...
	setwd("~/CENTER/SGP/California")
    source('/Volumes/Data/Dropbox/CENTER/SGP/California/PUC_2012/PUC_Course_Progressions_Math+ELA.R')

# Switch GRADE back to original grade values
	PUC_SGP@Data[['GRADE']] <- PUC_SGP@Data[['TMP_GRADE']]
	PUC_SGP@Data[['TMP_GRADE']] <- NULL
...
PUC.results <- .mergeSGP(HS.results, Elem.MS.results) # we'll put the HS results first

#  Merge this tmp list in with the PUC_SGP object:
PUC_SGP@SGP <- .mergeSGP(PUC_SGP@SGP, PUC.results)

#  Verify that we have the same results:
tot<-0
for(i in names(PUC_SGP@SGP[["SGPercentiles"]])[order(names(PUC_SGP@SGP[["SGPercentiles"]]))]) {
	print(paste(i, "N =", dim(PUC_SGP@SGP[["SGPercentiles"]][[i]])[1], " :  Median SGP,", median(PUC_SGP@SGP[["SGPercentiles"]][[i]][["SGP"]])))
	tot <- tot+(dim(PUC_SGP@SGP[["SGPercentiles"]][[i]])[1])
}
tot
...
#########################################################
	...
	PUC_SGP <- combineSGP(PUC_SGP, state="CA")

#  Check the LONG @Data file now:
tot<-0
for(i in names(PUC_SGP@SGP[["SGPercentiles"]])[order(names(PUC_SGP@SGP[["SGPercentiles"]]))]) {
	print(paste(dim(eval(parse(text=paste("PUC_SGP@SGP[['SGPercentiles']]$", i, sep=""))))[1], ": ", i))
	tot <- tot+(dim(eval(parse(text=paste("PUC_SGP@SGP[['SGPercentiles']]$", i, sep=""))))[1])
}
tot
sum(!is.na(PUC_SGP@Data$SGP)) # Should match up with 'tot' - 27,438

table(!is.na(PUC_SGP@Data$SGP), PUC_SGP@Data$CONTENT_AREA, PUC_SGP@Data$YEAR)
...
subjects <- c('ELA', 'HISTORY', 'US_HISTORY', 'WORLD_HISTORY', 'MATHEMATICS', 'GENERAL_MATHEMATICS', 'ALGEBRA_I', 'ALGEBRA_II', 'GEOMETRY', 'SUMMATIVE_HS_MATHEMATICS', 'SCIENCE', 'BIOLOGY', 'LIFE_SCIENCE', 'CHEMISTRY', 'PHYSICS') #  'INTEGRATED_SCIENCE_1',  

PUC_SGP <- summarizeSGP(PUC_SGP, state="CA",
		content_areas = subjects,
		years='2012', # might want more than just 2012 here?
		parallel.config=list(BACKEND="PARALLEL", WORKERS=8))

save(PUC_SGP, file="Data/PUC_SGP.Rdata", compress="bzip2")
...
##  NOTE:  this produces a file named California_SGP_LONG_Data.txt, which I've renamed PUC_SGP_LONG_Data.txt

outputSGP(PUC_SGP, state="CA", output.type="LONG_Data", outputSGP.directory="PUC_Results_2012")

###  Output summary tables

dir.create(paste("PUC_Results_2012/Summaries", sep=""), recursive=TRUE, showWarnings=FALSE)

###  Teacher only
s <- data.table(PUC_SGP@Summary[['DISTRICT_NUMBER']][['DISTRICT_NUMBER__INSTRUCTOR_NUMBER__CONTENT_AREA__YEAR']])
tmp <- data.table(s[!is.na(MEDIAN_SGP) & INSTRUCTOR_NUMBER!="NA",], key = c('INSTRUCTOR_NUMBER', 'CONTENT_AREA', 'YEAR'))
write.table(tmp, file=paste("PUC_Results_2012/Summaries/INSTRUCTOR_NUMBER__CONTENT_AREA__YEAR.txt", sep=""), sep="|", row.names=FALSE)

###  Teacher Grade Subject
s <- data.table(PUC_SGP@Summary[['DISTRICT_NUMBER']][['DISTRICT_NUMBER__INSTRUCTOR_NUMBER__CONTENT_AREA__YEAR__GRADE']])
tmp <- data.table(s[!is.na(MEDIAN_SGP) & INSTRUCTOR_NUMBER!="NA",], key = c('INSTRUCTOR_NUMBER', 'CONTENT_AREA', 'GRADE', 'YEAR'))
write.table(tmp, file=paste("PUC_Results_2012/Summaries/INSTRUCTOR_NUMBER__CONTENT_AREA__YEAR__GRADE.txt", sep=""), sep="|", row.names=FALSE)
...
#########################################################
	...
	setnames(PUC_SGP@Data, 'DISTRICT_NAME', 'CA_DISTRICT_NAME')
	setnames(PUC_SGP@Data, 'CMO_NAME', 'DISTRICT_NAME')
	
	visualizeSGP(sgp_object=PUC_SGP, state="CA", 
	...
	visualizeSGP(PUC_SGP, state="CA", 
			plot.types="bubblePlot",
			bPlot.styles=c(10),
			bPlot.years=c('2010', '2011', '2012'),
			bPlot.content_areas=c('WORLD_HISTORY', 'US_HISTORY', 'HISTORY', "SCIENCE", 'BIOLOGY', 'CHEMISTRY', 'PHYSICS',
